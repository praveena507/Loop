...
export async function initDatabase() {
  try {
    ...
    `);
    try { await dbRun(`ALTER TABLE staff_users ADD COLUMN plainPassword TEXT`); } catch (e) {}
    ...
    for (const col of aiCols) {
      try { await dbRun(`ALTER TABLE ai_analysis ADD COLUMN ${col} TEXT`); } catch (e) {}
    }
    ...
    console.log('LOOP Supabase & Local Database schema confirmed ready.');
  } catch (err) {
